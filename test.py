...
class App(tk.Tk):
    ...
    def check_answer(self):
        ...
        selected_answer = self.selected_answer.strip().lower()
        ...
        for idx, option in enumerate(question['options']):
            if option.strip().lower() == correct_answer:
                self.buttons[idx].configure(bg="green")
            elif option.strip().lower() == selected_answer:
        ...
        if selected_answer == correct_answer:
            ...
            score_feedback = f"Incorrect. Correct answer: {correct_answer.capitalize()}"
